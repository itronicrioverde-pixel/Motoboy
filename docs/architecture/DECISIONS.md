...
- Quando a tentativa **não** tem `clientName` e a entrada `{id}` já existe, `clientName`/`desc` **não** são comparados (o nome derivaria do cadastro atual, possivelmente renomeado): a validação fica restrita a cliente, valor, data e origem, e o nome e os timestamps da entrada persistida prevalecem, sem nova escrita.
